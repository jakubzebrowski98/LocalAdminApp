...
export default function useIngredients() {
    ...
    const getMealsIngredients = async (MealId) =>{
        let res = await axios.get('/api/mealsIngredients/' + MealId);
    ...
    const storeMealsIngredients = async (data) => {
        errors.value = ''
        try {
            await axios.post('/api/mealsIngredients/store', data)
        } catch (e) {
    ...
    const destroyMealsIngredients = async (MealIngId) => {
        await axios.delete('/api/mealsIngredients/' + MealIngId + '/delete');
    }
